...
export default class App {
  constructor(refs) {
    ...
    // ✅ zoom/pan do mapa (aplica no WRAP, não na imagem)
    this._mapZoom = 1;
    this._mapZoomMin = 1;
    this._mapZoomMax = 6;
  }
  ...
  _setupMapOverlay() {
    ...
    // ✅ zoom com wheel no desktop (âncora no cursor)
    if (body && wrap) {
  ...
  _setMapOpen(open) {
    ...
    if (this._mapOpen) {
      ...
      this._resetMapZoom(); // ✅ garante estado limpo ao abrir
      this._updateMapMarker();
    }
  }
  ...
  _ensureMapImageForTour() {
    ...
    if (this.ui.mapImg.src && this.ui.mapImg.src.endsWith(png)) {
      return;
    }
  ...
  // ✅ zoom no WRAP (img + marker juntos => alinhamento perfeito)
  ...
  async goToScene(sceneId, opts = {}) {
    ...
    // Atualiza marker mesmo com overlay fechado (estado fica pronto)
    this._updateMapMarker();
